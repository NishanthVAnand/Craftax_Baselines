...
class ActorCriticConvSymbolicCraftax(nn.Module):
    ...
    def __call__(self, obs, llm_embedding):
        ...
        image_embedding = nn.max_pool(image_embedding, window_shape=(2, 2), strides=(1, 1))
        ...
        # Combine embeddings
        embedding = jnp.concatenate([image_embedding, flat_obs, llm_embedding], axis=-1)
        embedding = nn.Dense(self.layer_width, kernel_init=orthogonal(2), bias_init=constant(0.0))(
            embedding
        )
        embedding = nn.relu(embedding)

        actor_mean = nn.Dense(self.layer_width, kernel_init=orthogonal(2), bias_init=constant(0.0))(
            embedding
        )
        ...
        critic = nn.Dense(self.layer_width, kernel_init=orthogonal(2), bias_init=constant(0.0))(
            embedding
        )
        ...
        critic = jnp.concatenate([critic, llm_embedding], axis=-1)
...
class ActorCriticConv(nn.Module):
    ...
    def __call__(self, obs, llm_embedding):
        ...
        embedding = jnp.concatenate([embedding, llm_embedding], axis=-1)
...
class ActorCriticWithEmbedding(nn.Module):
    ...
    def __call__(self, x, llm_embedding):
        ...
        actor_emb = nn.Dense(128, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(
            actor_emb
        )
        actor_emb = activation(actor_emb)
...
class ActorCriticLinear(nn.Module):
    ...
    def __call__(self, x, llm_embedding):
        ...
        critic = nn.Dense(1, kernel_init=orthogonal(1.0), bias_init=constant(0.0))(concat_input)

        return pi, jnp.squeeze(critic, axis=-1)


class ActorCriticOneLayer(nn.Module):
    action_dim: Sequence[int]
    layer_width: int
    activation: str = "tanh"
...
class ActorCriticTwoLayer(nn.Module):
    action_dim: Sequence[int]
    layer_width: int
    activation: str = "tanh"
